...
def get_location(search_term)
  response = HTTParty.get(BASE_URL, query: { key: LOCATION_IQ_KEY, q: search_term, format: "json" })

  raise ArgumentError.new("Cannot find location") if response.code != 200

  location = {}
  location[search_term] = {search_term => { lat: response.first["lat"], lon: response.first["lon"] }}
end
